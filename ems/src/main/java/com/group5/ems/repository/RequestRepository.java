...
public interface RequestRepository extends JpaRepository<Request, Long> {
    ...
    @Query(value = """
			...
			""",
            countQuery = """
			SELECT COUNT(r) FROM Request r JOIN r.requestType rt
			WHERE r.status NOT IN ('PENDING', 'CANCELLED') AND rt.category = 'ATTENDANCE'
			""")
    Page<Request> findLeaveHistory(Pageable pageable);
        ...
        @Query(value = """
			SELECT r FROM Request r JOIN r.employee e JOIN e.user u
			LEFT JOIN e.department d JOIN r.requestType rt
			WHERE rt.category = 'HR_STATUS' ORDER BY r.createdAt DESC
			""",
            countQuery = """
			SELECT COUNT(r) FROM Request r JOIN r.requestType rt WHERE rt.category = 'HR_STATUS'
			""")
        Page<Request> findWorkflowRequests(Pageable pageable);

        // ── Additional workflow queries ──
	@Query("""
			SELECT r FROM Request r
			JOIN FETCH r.employee e
			JOIN FETCH e.user u
	...
	@Query("""
			SELECT COUNT(r) FROM Request r WHERE r.status = 'APPROVED'
    ...
    List<Request> findRecentRequestsByCategory(
         ...
         */
	@Query("""
			SELECT r FROM Request r JOIN r.requestType rt
    ...
    @Query("""
			SELECT COUNT(r) FROM Request r WHERE r.status = :status AND r.step = 'WAITING_HR' AND r.requestType.code IN :codes
			""")
    ...
    @Query("""
			SELECT COUNT(r) FROM Request r JOIN r.requestType rt WHERE rt.category = :category
			""")
    ...
    @Query("""
			SELECT COUNT(r) FROM Request r JOIN r.requestType rt WHERE rt.category <> :excludeCategory
			""")
    ...
    @Query(value = """
			...
			""",
            countQuery = """
			SELECT COUNT(r) FROM Request r JOIN r.employee e JOIN e.user u
			LEFT JOIN e.department d JOIN r.requestType rt
			WHERE r.status NOT IN ('PENDING', 'CANCELLED') AND rt.category = 'ATTENDANCE'
			AND (:status IS NULL OR r.status = :status)
			AND (:departmentId IS NULL OR d.id = :departmentId)
