...
public interface RequestRepository extends JpaRepository<Request, Long> {
        ...
        Optional<Request> findByIdAndEmployeeDepartmentIdAndLeaveTypeIsNotNull(Long id, Long departmentId);
        ...
        @Query(value = "SELECT r FROM Request r JOIN r.employee e JOIN e.user u " +
                        ...
                        "WHERE r.status <> 'PENDING' AND rt.category = 'ATTENDANCE' " +
                        "ORDER BY r.createdAt DESC", countQuery = "SELECT COUNT(r) FROM Request r JOIN r.requestType rt "
                                        +
                                        "WHERE r.status <> 'PENDING' AND rt.category = 'ATTENDANCE'")
